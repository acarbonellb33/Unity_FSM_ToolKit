...
namespace EditorWindow.FSMSystem.Inspectors
{
	...
	[CustomEditor(typeof(AmbushTest))]
	public class AmbushTestEditor : Editor
	{
		...
		void OnEnable()
		{
			...
			AmbushTest ambushTest = (AmbushTest)target;
			Type type = typeof(AmbushTest);
			...
			foreach (FieldInfo field in fields)
			{
				...
				try
				{
					optionToObjectMap[nameField] = ambushTest.options[j];
				}
				...
				while (nextVisible)
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						if(selectedObject.GetStateName().StartsWith("CustomCondition"))
						{
							...
							FsmIOUtility.CreateJson(selectedObject, "AmbushTest");
						}
