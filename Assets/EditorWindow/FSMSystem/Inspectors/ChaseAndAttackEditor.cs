...
namespace EditorWindow.FSMSystem.Inspectors
{
	...
	[CustomEditor(typeof(ChaseAndAttack))]
	public class ChaseAndAttackEditor : Editor
	{
		...
		void OnEnable()
		{
			...
			ChaseAndAttack chaseAndAttack = (ChaseAndAttack)target;
			Type type = typeof(ChaseAndAttack);
			...
			foreach (FieldInfo field in fields)
			{
				...
				try
				{
					optionToObjectMap[nameField] = chaseAndAttack.options[j];
				}
				...
				while (nextVisible)
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						if(selectedObject.GetStateName().StartsWith("CustomCondition"))
						{
							...
							FsmIOUtility.CreateJson(selectedObject, "ChaseAndAttack");
						}
