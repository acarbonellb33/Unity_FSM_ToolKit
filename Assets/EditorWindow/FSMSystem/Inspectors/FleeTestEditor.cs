...
namespace EditorWindow.FSMSystem.Inspectors
{
	...
	[CustomEditor(typeof(FleeTest))]
	public class FleeTestEditor : Editor
	{
		...
		void OnEnable()
		{
			...
			FleeTest fleeTest = (FleeTest)target;
			Type type = typeof(FleeTest);
			...
			foreach (FieldInfo field in fields)
			{
				...
				try
				{
					optionToObjectMap[nameField] = fleeTest.options[j];
				}
				...
				while (nextVisible)
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						if(selectedObject.GetStateName().StartsWith("CustomCondition"))
						{
							...
							FsmIOUtility.CreateJson(selectedObject, "FleeTest");
						}
