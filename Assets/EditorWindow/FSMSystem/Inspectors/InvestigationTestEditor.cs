...
namespace EditorWindow.FSMSystem.Inspectors
{
	...
	[CustomEditor(typeof(InvestigationTest))]
	public class InvestigationTestEditor : Editor
	{
		...
		void OnEnable()
		{
			...
			InvestigationTest investigationTest = (InvestigationTest)target;
			Type type = typeof(InvestigationTest);
			...
			foreach (FieldInfo field in fields)
			{
				...
				try
				{
					optionToObjectMap[nameField] = investigationTest.options[j];
				}
				...
				while (nextVisible)
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						if(selectedObject.GetStateName().StartsWith("CustomCondition"))
						{
							...
							FsmIOUtility.CreateJson(selectedObject, "InvestigationTest");
						}
