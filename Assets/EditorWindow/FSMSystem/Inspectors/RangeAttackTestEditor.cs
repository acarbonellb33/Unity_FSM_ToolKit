...
namespace EditorWindow.FSMSystem.Inspectors
{
	...
	[CustomEditor(typeof(RangeAttackTest))]
	public class RangeAttackTestEditor : Editor
	{
		...
		void OnEnable()
		{
			...
			RangeAttackTest rangeAttackTest = (RangeAttackTest)target;
			Type type = typeof(RangeAttackTest);
			...
			foreach (FieldInfo field in fields)
			{
				...
				try
				{
					optionToObjectMap[nameField] = rangeAttackTest.options[j];
				}
				...
				while (nextVisible)
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						if(selectedObject.GetStateName().StartsWith("CustomCondition"))
						{
							...
							FsmIOUtility.CreateJson(selectedObject, "RangeAttackTest");
						}
