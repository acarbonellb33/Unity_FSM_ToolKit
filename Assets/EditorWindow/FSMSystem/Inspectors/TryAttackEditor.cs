...
[CustomEditor(typeof(TryAttack))]
public class TryAttackEditor : Editor
{
	...
	void OnEnable()
	{
		...
		TryAttack tryAttack = (TryAttack)target;
		Type type = typeof(TryAttack);
		FieldInfo[] fields = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
		int j = 0;
		foreach (FieldInfo field in fields)
		{
			string nameField = FixName(field.Name);
			optionToObjectMap[nameField] = tryAttack.options[j];
			j++;
		}
	}
			...
			while (nextVisible)
			{
				if (iterator.name != "m_Script")
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						FSMIOUtility.CreateJson(selectedObject, "TryAttack");
					}
				}
