...
namespace EditorWindow.FSMSystem.Inspectors
{
	...
	[CustomEditor(typeof(chasingai))]
	public class chasingaiEditor : Editor
	{
		...
		void OnEnable()
		{
			...
			chasingai chasingai = (chasingai)target;
			Type type = typeof(chasingai);
			...
			foreach (FieldInfo field in fields)
			{
				...
				try
				{
					optionToObjectMap[nameField] = chasingai.options[j];
				}
				...
				while (nextVisible)
				{
					if (iterator.name != "m_Script")
					{
						...
						if (EditorGUI.EndChangeCheck())
						{
							...
							FsmIOUtility.CreateJson(selectedObject, "chasingai");
						}
					}
