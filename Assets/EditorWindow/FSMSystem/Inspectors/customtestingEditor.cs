...
[CustomEditor(typeof(customtesting))]
public class customtestingEditor : Editor
{
	...
	void OnEnable()
	{
		...
		customtesting customtesting = (customtesting)target;
		Type type = typeof(customtesting);
		FieldInfo[] fields = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
		int j = 0;
		foreach (FieldInfo field in fields)
		{
			string nameField = FixName(field.Name);
			optionToObjectMap[nameField] = customtesting.options[j];
			j++;
		}
	}
			...
			while (nextVisible)
			{
				if (iterator.name != "m_Script")
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						FSMIOUtility.CreateJson(selectedObject, "customtesting");
					}
				}
