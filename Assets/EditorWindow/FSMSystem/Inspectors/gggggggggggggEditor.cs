...
[CustomEditor(typeof(ggggggggggggg))]
public class gggggggggggggEditor : Editor
{
	...
	void OnEnable()
	{
		...
		ggggggggggggg ggggggggggggg = (ggggggggggggg)target;
		Type type = typeof(ggggggggggggg);
		FieldInfo[] fields = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
		int j = 0;
		foreach (FieldInfo field in fields)
		{
			string nameField = FixName(field.Name);
			optionToObjectMap[nameField] = ggggggggggggg.options[j];
			j++;
		}
	}
			...
			while (nextVisible)
			{
				if (iterator.name != "m_Script")
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						FSMIOUtility.CreateJson(selectedObject, "ggggggggggggg");
					}
				}
