...
[CustomEditor(typeof(kkkkkkkkkkk))]
public class kkkkkkkkkkkEditor : Editor
{
	...
	void OnEnable()
	{
		...
		kkkkkkkkkkk kkkkkkkkkkk = (kkkkkkkkkkk)target;
		Type type = typeof(kkkkkkkkkkk);
		FieldInfo[] fields = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
		int j = 0;
		foreach (FieldInfo field in fields)
		{
			string nameField = FixName(field.Name);
			optionToObjectMap[nameField] = kkkkkkkkkkk.options[j];
			j++;
		}
	}
			...
			while (nextVisible)
			{
				if (iterator.name != "m_Script")
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						FSMIOUtility.CreateJson(selectedObject, "kkkkkkkkkkk");
					}
				}
