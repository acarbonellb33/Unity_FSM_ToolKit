...
namespace EditorWindow.FSMSystem.Inspectors
{
	...
	public class meleeAttackEditor : Editor
	{
		...
		void OnEnable()
		{
			...
			meleeAttack meleeAttack = (meleeAttack)target;
			Type type = typeof(meleeAttack);
			...
			foreach (FieldInfo field in fields)
			{
				...
				try
				{
					optionToObjectMap[nameField] = meleeAttack.options[j];
				}
				...
				while (nextVisible)
				{
					if (iterator.name != "m_Script")
					{
						...
						if (EditorGUI.EndChangeCheck())
						{
							...
							FsmIOUtility.CreateJson(selectedObject, "meleeAttack");
						}
					}
