...
[CustomEditor(typeof(pelople))]
public class pelopleEditor : Editor
{
	...
	void OnEnable()
	{
		...
		pelople pelople = (pelople)target;
		Type type = typeof(pelople);
		FieldInfo[] fields = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
		int j = 0;
		foreach (FieldInfo field in fields)
		{
			string nameField = FixName(field.Name);
			optionToObjectMap[nameField] = pelople.options[j];
			j++;
		}
	}
					...
					if (iterator.isArray)
					{
						...
						for (int i = 0; i < iterator.arraySize; i++)
						{
							...
							if (gameObjectElementProperty.objectReferenceValue != null)
							{
								...
								FSMIOUtility.CreateJson(selectedObject, "pelople");
							}
							EditorGUILayout.EndHorizontal();
						}
						if (GUILayout.Button("Create and Add a Patrol Point"))
						{
							CreateAndAddGameObject(pelople);
						}
					}
					else
					{
	...
	private void CreateAndAddGameObject(pelople pelople)
	{
		pelople.AddObjectToList();
	}
	private void RemovePatrolPoint(GameObject patrolPoint)
	{
		pelople pelople = (pelople)target;
		pelople.patrol.RemovePatrolPoint(patrolPoint);
