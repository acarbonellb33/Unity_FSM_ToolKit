...
using UnityEngine;
[CustomEditor(typeof(yrtyfdghrtghdxfg))]
public class yrtyfdghrtghdxfgEditor : Editor
{
	...
	void OnEnable()
	{
		...
		yrtyfdghrtghdxfg yrtyfdghrtghdxfg = (yrtyfdghrtghdxfg)target;
		for (int i = 0; i < yrtyfdghrtghdxfg.options.Count; i++)
		{
			optionToObjectMap[yrtyfdghrtghdxfg.options[i].GetStateName()] = yrtyfdghrtghdxfg.options[i];
		}
	}
	public override void OnInspectorGUI()
	{
		...
		yrtyfdghrtghdxfg yrtyfdghrtghdxfg = (yrtyfdghrtghdxfg)target;
		string[] options = new string[yrtyfdghrtghdxfg.options.Count];
		for (int i = 0; i < options.Length; i++)
		{
			options[i] = yrtyfdghrtghdxfg.options[i].GetStateName();
		}
			...
			while (nextVisible)
			{
				if (iterator.name != "m_Script")
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						...
						FSMIOUtility.CreateJson(selectedObject, "yrtyfdghrtghdxfg");
					}
				}
