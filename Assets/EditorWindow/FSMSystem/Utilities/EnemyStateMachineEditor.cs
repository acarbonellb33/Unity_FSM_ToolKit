...
public static class EnemyStateMachineEditor
{
    ...
    private static string GenerateEditorScriptContent(FSMGraphSaveData saveData)
    {
        ...
        if (hasPatrolState)
        {
            ...
            scriptContent += "\t\t\t\t\t\t\t\tEditorGUI.BeginChangeCheck();\n";
            scriptContent += "\t\t\t\t\t\t\t\tgameObject = EditorGUILayout.ObjectField(\"Patrol Point\", gameObject, typeof(GameObject), true) as GameObject;\n";
            scriptContent += "\t\t\t\t\t\t\t\tif (EditorGUI.EndChangeCheck())\n";
            scriptContent += "\t\t\t\t\t\t\t\t{\n";
            scriptContent += "\t\t\t\t\t\t\t\t\tif (gameObject != null)\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t{\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t\tIDGenerator idGenerator = gameObject.GetComponent<IDGenerator>();\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t\tif (idGenerator.GetUniqueID() != null)\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t\t{\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t\t\tgameObjectElementProperty.stringValue = idGenerator.GetUniqueID();\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t\t}\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t}\n";
            scriptContent += "\t\t\t\t\t\t\t\t\telse\n";
            scriptContent += "\t\t\t\t\t\t\t\t\t{\n";
